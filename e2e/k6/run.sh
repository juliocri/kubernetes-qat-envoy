    ...
    for CIPHER in ${CIPHERS[@]}; do
      ...
      if [ "$RUN" == "k8s" ]; then
        ...
        if [ -n "$CLIENT" ]; then
          ...
          ssh -i ./key.pem -oStrictHostKeyChecking=no ${CLIENT} "docker run --net=host -i loadimpact/k6:master run --vus 30 --duration 20s -< /tmp/k6-testing-config-docker.js" | tee ${CDIR}/ciphers/${CIPHER}/results.txt;
          ssh -i ./key.pem -oStrictHostKeyChecking=no ${CLIENT} "rm -rf /tmp/k6-testing-config-docker.js"
        else
          docker run --net=host -i loadimpact/k6:master run --vus 30 --duration 20s -< ./tests/k6-testing-config-docker.js | tee ${CDIR}/ciphers/${CIPHER}/results.txt;
        fi
      fi
