...
if [ "$STATUS" == "Running" ]; then
  ...
  if [ -n "$CLIENT" ]; then
    CODE=$(ssh -i ./key.pem -oStrictHostKeyChecking=no ${CLIENT} "curl -islk https://$HOSTIP:$SVC_PORT | grep HTTP | awk '{print \$2}'");
  else
    CODE=$(curl -islk  https://127.0.0.1:$SVC_PORT | grep HTTP | awk '{print $2}');
  fi
